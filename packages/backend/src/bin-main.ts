...
const program = new Command()
  .name("relay-server")
  .description("Start the content-relay backend server")
  .showHelpAfterError()
  .addOption(
...
const serverBaseUrl = options.baseUrl ?? `http://127.0.0.1:${options.port}`;
...
await runWithDiContainer(diContainer, async () => {
  try {
    ...
    logger.info(`relay-server listening on ${serverBaseUrl} using ${options.dataDir}`, {
      port: options.port,
      dataDirectory: options.dataDir,
      serverBaseUrl,
    });
  } catch (error) {
    logger.error({ error }, `Failed to start relay-server`);
